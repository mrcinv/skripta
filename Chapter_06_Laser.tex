...
frekvenčnega zamika $\nu_0$ so frekvence izhodne svetlobe zelo 
...
$10^{-15}~\si{Hz}$.\footnote{Za odkritje 
...
v fazi, zaradi katerega vrh ovojnice sunka na splošno ne sovpada z vrhom amplitude
nihanja (slika~\ref{fig:comb2}). 
Če uspemo ohranjati ta zamik konstanten znotraj preleta, se pri izrazu za
frekvenco izhodne svetlobe pojavi dodatni zamik $\nu_0 \neq 0$ (enačba~\ref{eq:comb}). 
Za absolutno
določitev frekvence moramo ta zamik seveda natačno poznati. 
\begin{figure}[h]
\centering
...
osnovni in pri podvojeni frekvenci\footnote{Interferometer se imenuje $f$--$2f$
interferometer, kar nakazuje na osnovno ($f$)  in podvojeno ($2f$) frekvenco.}.
To dosežemo z nelinearnimi optičnimi pojavi, ki omogočajo generacijo valovanja 
pri podvojeni frekvenci vpadnega (poglavje~\ref{chap:SHG}). Podvojeno frekvenco, ki je
pri $2(\nu_0 + m\Delta \nu)= 2\nu_0 + 2m\Delta \nu$ primerjamo s frekvenco pri 
točno dvakrat večjem $m$, ki znaša $\nu_0 + 2m\Delta \nu$. Razlika med tema 
dvema frekvencama je ravno $\nu_0$, ki smo jo iskali. S povratno zanko jo 
lahko vzdržujemo konstantno. Ko določimo oba parametra $\nu_0$ in $\Delta \nu$, 

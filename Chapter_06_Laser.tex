...
V prejšnjih poglavjih smo spoznali resonatorje, pojasnili ojačevanje svetlobe in 
opisali črpanje, ki je potrebno za ojačevanje optičnega signala. V tem poglavju bomo vsa ta
spoznanja združili in komponenete sestavili v eno samo napravo -- laser. Opisali 
bomo delovanje laserjev in spoznali njihove prednosti pred navadnimi svetili. 
Zapisali bomo zasedbene enačbe in razložili delovanje laserjev, spoznali zanimive
spremljujoče pojave, opisali načine delovanja laserjev v sunkih in 
na koncu predstavili še semiklasični model laserja. 

\section{Laser}
\index{Laser}
Spoznali smo, da se svetloba pri prehodu skozi sredstvo z obrnjeno zasedenostjo
med dvema nivojema ojačuje. Postavimo tako sredstvo v optični resonator. 
\index{Resonator} Na začetku nastaja predvsem spontano izsevana svetloba, ki 
\index{Spontano sevanje} se med zrcaloma
resonatorja odbija in ob vsakem preletu skozi snov postopoma ojačuje.
\index{Optično ojačevanje}
Vzbujajo se nihanja resonatorja z nihajnimi frekvencami blizu frekvence
...
\caption{Shema laserja s ključnimi deli: ojačevalno sredstvo, 
črpalni mehanizem in resonator}
...
črpalni mehanizem in ukrivljeni zrcali, ki tvorita resonator 
(slika~\ref{fig:shemalaserja}). Črpalni \index{Črpanje}
mehanizem vzdržuje obrnjeno zasedenost v ojačevalnem sredstvu, resonator pa omogoča,
da se svetloba dovolj ojači in tako stimulirano sevanje prevlada nad spontanim. 
Odbojnost vsaj enega od zrcal mora biti manjša od 1, da skozenj lahko izhaja svetloba.

Pri osnovnem opisu delovanja laserja se omejimo na najpreprostejši model in 
privzemimo, da frekvenca le enega resonatorskega nihanja sovpada s 
...
sredstva.\index{Saturacijska energija}
Račun pogosto poenostavimo, tako da vzamemo $L'=L$, tukaj pa zaradi
...
Za podrobnejši opis delovanja laserja zapišimo zasedbene enačbe. 
...
energijo lastnega valovanja v resonatorju. Še naprej obravnavajmo primer, ko je 
...
$n$, ki opisuje energijo v izbranem stanju resonatorja. Gostota energije
polja je $w = n\hbar\omega/V$, pri čemer je $V$ volumen resonatorja. 
...
gornjega atomskega stanja. Enačbi sta nelinearni in ju ne znamo 
...
\label{5.10}
\end{equation}
V zadnjem izrazu smo privzeli, da je $g\simeq 1/\Delta \omega $. 
Parameter $p$ je torej približno enak produktu 
...
\begin{remark}
S primerjavo izraza za $p$ 
(enačba~\ref{5.10}) z izrazom za saturacijsko gostoto toka (enačba~\ref{eq:jsatg})
vidimo, da $p$ predstavlja število fotonov v resonatorju, pri katerem pride 
...
preveč blizu praga, ko je $G/G_\mathrm{pr}\simeq 1$. 
...
Izjema so polprevodniški laserji, katerih volumen -- in posledično tudi $p$ -- 
...
resonatorju je polje po obliki zelo blizu Gaussovem snopu, zato je tak tudi izhodni snop.
...
sorazmerna s korenom iz števila fotonov v izbranem nihanju. 
...
Povzemimo, kar smo se do zdaj naučili o laserjih in jih primerjajmo z navadnimi svetili. 
...
Tako se lahko zgodi, da ojačenje v laserju za več nihanj hkrati 
presega ojačenje na pragu in vzbujenih je več nihanj.
...
potrebujemo ozko spektralno črto. Zato moramo poskrbeti, da je vzbujeno le
eno samo nihanje resonatorja, najbolje tisto, ki je najbližje vrhu ojačenja
aktivnega sredstva. To dosežemo tako, da za vsa ostala nihanja povečamo izgube,
na primer s Fabry-Perotovim etalonom, ki ga vstavimo v laserski resonator
(slika~\ref{fig:FPres}). Njegova prepustnost v odvisnosti od krožne frekvence 
$\omega$, debeline $L$, lomnega količnika $n$, odbojnost sten ${\cal R}$ in 
nagiba glede na os resonatorja $\varphi$ je podana z enačbo~(\ref{eq:FP}) 
\begin{equation}
T=\frac{1}{1+\frac{4{\cal {R}}}{(1-{\cal {R}})^{2}}\sin^{2}(\frac{n\omega}{c}L\cos \varphi )}
\label{5.22}
\end{equation}
in jo kaže slika~(\ref{fig:Fabry-Perot}). 
...
nestacionarnega delovanja pa se moramo vrniti k obravnani zasedbenih enačb 
...
\label{5.28}
\end{equation}
Zapisan linearni sistemi diferencialnih enačb s konstantnimi
...
dušenim nihanjem, ki mu pravimo relaksacijske oscilacije. Primer takega nihanja 
pri vključitvi laserja kaže slika~(\ref{fig:relax}).
\begin{figure}[h]
\centering
\def\svgwidth{80truemm} 
...
Obravnavajmo najprej najpreprostejši način, pri katerem moduliramo črpanje.
...
koeficientav $c_1(t)$ in $c_2(t)$ zvezi
(enačbi~\ref{eq:c1c2})
\begin{equation}
...
\begin{equation}  
\label{5.50}
\dot{\zeta}=-\frac{2i}{\hbar}\, E(t)v_{12}(\eta^{\ast}- \eta).
\end{equation}
...
\begin{equation}  
\label{5.51}
\dot{\zeta}=A(\zeta_0-\zeta)- \frac{2i}{\hbar}\,E(t)v_{12}(\eta^{\ast}-\eta),
\end{equation}
...
\begin{equation}  
\label{5.52}
\dot{\eta}=- (i \omega_0+\gamma)\eta+\frac{i}{\hbar}\,E(t)v_{12} \zeta
\end{equation}
...
\begin{equation}  
\label{5.53}
P=\frac{N}{V}p = \frac{N}{V}\,v_{12}(\eta+\eta^{\ast})=P^+ + P^-.
\end{equation}
...
prepišemo v obliko 
\boxeq{5.56}{
\frac{dZ}{dt} &= A(Z_0-Z)-\frac{2i}{\hbar}E(P^- - P^+), \\
\frac{dP^+}{dt}&=(-i \omega_0-\gamma)P^{+}+\frac{i}{\hbar} Ev_{12}^2 Z, \label{5.56b}\\
\frac{dP^-}{dt}&=(i \omega_0-\gamma)P^{-}+\frac{i}{\hbar} Ev_{12}^2 Z.\label{5.56c}
}
...
delovanju je periodična, vendar njena krožna frekvenca ni nujno enaka lastni
krožni frekvenci praznega resonatorja $\omega_{\lambda}$. Krožno frekvenco 
$\omega_\lambda$ moramo še izračunati.
...
kar moramo prostorsko povprečiti. Funkcije $u_{\lambda}(\vec{r})$ so
normirane, tako da je $\int u_{\lambda}^2(\vec{r}) \,dV=V$. Tako imamo $%

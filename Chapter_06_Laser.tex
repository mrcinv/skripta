...
je prag za delovanje laserja in \index{Obrnjena zasedenost}\index{Izgube v resonatorju}
večja je lahko stopnja obrnjene zasedenosti. Tako v sistemu atomov shranimo več 
energije, ki se lahko izseva iz sistema preko stimuliranega sevanja. \index{Sunkovni laser}
...
Izračunamo lahko tudi dolžino sunkov (glej nalogo~\ref{naloga:mlock})
\begin{equation}
...
frekvenc med lastnimi resonatorskimi stanji. Za ta namen dodamo v resonator
modulator, na primer akusto-optični modulator (glej poglavje~\ref{chap:modulacija}).
Ko v akusto-optičnemu modulatorju vzbudimo stoječe zvočno valovanje, se 
svetloba na njem uklanja in izgube so velike. V časovnih razmikih $T=2L/c$
je amplituda stoječega vala enaka nič in uklonske izgube se zmanjšajo.
V resonatorju se tako uspešno ojačuje le kratek sunek svetlobe. 
Navadno zadošča razmeroma majhna sinusna modulacija izgub, kjer je relativna 
prepustnost v minimumu za nekaj destink manjša od maksimalne. 

Kako pri modulaciji pride do uklepanja faz, lahko uvidimo še drugače.
Modulacija amplitude posameznega lastnega nihanja povzroči, da se v spektru nihanja
...
prištejeta, če imata enako fazo. S tem se zmanjšajo izgube in 
delovanje laserja z uklenjenimi fazami ima najnižji prag. 

Pri pasivnem uklepanju faz dodanega elementa ne krmilimo od zunaj, ampak
se sam ustrezno odziva. Ena od možnosti je plast barvila, ki močno
absorbira svetlobo pri majhni gostoti svetlobnega toka, pri veliki 
...
velikem ojačenju se bo ta izbrana fluktuacija ojačevala, ostale pa ne, zato
se spet pojavi fazno uklenjeni sunek. Ker mora po prehodu sunka absorpcija 
v barvilu hitro narasti, mora biti relaksacijski čas barvila zelo kratek
$\sim \si{ps}$.
In this scheme,
...
Drug način pasivnega uklepanja faz je z uporabo nelinearne optike (glej 
poglavje~\ref{}).
This lensing effect has a practical application: it is used to induce self-mode-
locking in solid-state lasers, such as titanium sapphire lasers. The mechanism is the
following. The multimode laser can operate either in a free-running regime, in which
the modes have random phases, or in a mode-locking regime. If the number of modes
is large, the peak intensity of the mode-locked pulses is much larger than the CW
intensity of the free-running laser. It is assumed that the third-order nonlinearity of
the active crystal gives rise to a Kerr lens in the mode-locking regime, but no Kerr lens
in free running. The effect of the Kerr lens is that of focusing the laser beam at some
point inside the cavity. If a pinhole is placed at that point, the free-running regime
will experience large losses, whereas the mode-locking regime will be unaffected.
As a consequence, the laser is forced to operate as a mode-locked oscillator. More
generally, Kerr-lens mode-locking can be explained by keeping in mind the definition
of cavity stability given in Sect. 3.8. If the cavity is designed to be stable in presence
of the Kerr lens and unstable in absence of the Kerr lens, the laser will choose to
operate in the mode-locking regime in presence of self-focusing. The technique is

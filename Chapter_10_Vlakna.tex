...
$k_xa$ in narišemo funkcijo $k_xa\, \tan (k_xa/2)$ (rdeče črte). Enačbo za lihe 
rodove~(enačba~\ref{sekular2}) preoblikujemo in narišemo $-k_xa\, \cot (k_xa/2)$ (črne črte).
Nato pri danih parametrih $n_1$, $n_2$, $a$ in $k_0$ narišemo krožnico za $\kappa a$, 
ki jo opisuje enačba~(\ref{kappak}). Število presečišč krožnice s krivuljami določa 
število lastnih rodov v izbranem vlaknu, lega presečišč pa določa vrednosti $k_x$
za posamezne lastne rodove. 
\begin{figure}[h]
\centering
...
v vodniku. Zelena krožnica prestavlja enorodovni vodnik, modra dvorodovnega in vijolična
petrodovnega.}
\label{fig:TEsec}
\end{figure}

Na grafu so narisane tri krožnice, ki pri istih lomnih količnikih in isti valovni dolžini 
svetlobe predstavljajo tri različne debeline vodnika. V najtanjšem vodniku (zelena črta) je presečišče
le eno in tak vodnik imenujemo enorodovni vodnik (in vlakno enorodovno 
vlakno)\index{Optični vodnik!enorodovni}. Z večanjem debeline število rodov narašča
in vodnik je večrodoven\index{Optični vodnik!večrodovni}. V primeru, označenem z vijolično
barvo, je rešitev pet: trije sodi rodovi in dva liha. 

Z grafa razberemo še eno pomembno 
lastnost. Ne glede na to, kako tanek je vodnik (kako majhen je krog), vedno obstaja vsaj ena 
rešitev za $k_x$, saj rdeča krivulja prvega roda seka še tako majhno krožnico. 
Za tipično enorodovno vlakno velja $a\lesssim 5~\si{\micro\meter}$, za večrodovno 
z okoli 20 rodovi pa $a\sim 50~\si{\micro\meter}$.

Ocenimo število lastnih rodov še z izračunom. S slike~(\ref{fig:TEsec}) vidimo, da je 
največja možna vrednost prečne komponente valovnega vektorja $k_x$ omejena s polmerom 
krožnice. Do te vrednosti je po ena rešitev na vsak interval 
dolžine $\pi/a$, izmenično soda in liha. Celotno število rodov za eno polarizacijo 
je tako\index{Optični vodnik!število rodov}
\begin{equation}
N \approx \frac{k_{x\mathrm{max}}}{\pi/a}  = \frac{k_0\, a\, NA }{\pi} = \frac{V}{\pi},
\end{equation}
pri  čemer smo vpeljali normirano frekvenco\index{Normirana frekvenca}, ki je ravno
enaka polmeru krožnice.
\begin{equation}
V = k_0a \sqrt{n_1^2-n_2^2} = k_0 a NA.
\end{equation}
...
($\sim 13~\si{THz}$), se signalni žarek ojača. Ker je spekter SiO$_2$ razmeroma 

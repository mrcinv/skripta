...
so osnovne značilnosti valovanja, ki se širi po vodniku ali vlaknu.
...
\label{9.2}
\end{equation}
Glede na smer vektorja $\bf{e}$ ločimo dva različna primera. Če leži električna poljska
jakost vzporedno z mejnima ploskvama $\mathbf{e} = (0,1,0)$, 
govorimo o transverzalnem električnem (TE) valovanju, v nasprotnem primeru, ko je 
z mejnima ploskvama vzporedna magnetna poljska jakost in $\mathbf{e} = (e_x,0,e_z)$, pa 
...
\label{fig:TETM}
\end{figure}
Omejimo se le na prvi primer, to je TE polarizacijo (za izračun lastnih rodov TM polariziranega
...
Robni pogoji so, da sta tangencialni

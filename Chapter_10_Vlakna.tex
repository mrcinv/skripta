...
Kadar je vodnik ukrivljen, del valovanja uhaja v plašč in 
...
Naj bo $z=R\phi$ dolžina loka vzdolž sredine sredice. Tedaj je faza nastavka
...
enačbe v sredici (\ref{9.3a}), ki so reda $1/R.$ Privzamimo, da je $1/r\approx1/R$ in 
\begin{equation}
...
\label{9.35}
\end{equation}
Dobljena enačba je zelo podobna enačbi za izračun lastnih rodov v planparalelnem
...
med lastnimi načini v valovnem vodniku in stanji delca, ki je ujet v končno potencialno jamo, 
je ta člen enake oblike kot potencialna energija delca v konstantnem zunanjem električnem 
polju (slika~\ref{fig:tunel}). Podobno kot v tem primeru delci uhajajo iz potencialne jame
...
A \propto e^{-CR},
\eeq
pri čemer je $C$ konstanta, odvisna od lomnih količnikov in polmera vlakna ter od 
valovne dolžine potujoče svetlobe.
\begin{figure}[h]
\centering
\def\svgwidth{120truemm} 
...
so podobni stanjem delca v konstantnem zunanjem električnem polju (desno).}
\label{fig:tunel}
\end{figure}
...
dani spremembi lomnega količnika.
...
in prečna oblika $\psi=\psi_{lm}+\delta\psi.$ Tudi popravljena funckija
...
enačbe s $\psi_{lm}^{*}$, integrirajmo po preseku vlakna in 
upoštevajmo, da je $\delta\psi$ ortogonalna na $\psi_{lm}$. Dobimo
\begin{eqnarray}
...
spremembe amplitude vpadnega vala $E_{0}$. Celtono odbito valovanje je vsota 
...
\eeq
Pokaži, da je amplitda odbitega valovanja v tem primeru enaka
\beq

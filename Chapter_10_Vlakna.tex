...
Sunek zapišimo kot  
\begin{equation}
E\left(x, y, z, t\right)=a\left(z,t\right)\,\psi\left(x,y\right),
\label{9.61}
\end{equation}
kjer je $\psi\left(x,y\right)$ lastna rešitev prečnega dela valovne
enačbe, ki določa tudi zvezo $\beta\left(\omega\right)$. 
Funkcijo $a\left(z,t\right)$, ki opisuje obliko sunka v smeri $z$ in tudi njegovo
širjenje, zapišemo s Fourierevim integralom po frekvencah 
\begin{equation}
a\left(z,t\right)=\int \tilde{a}(z, \omega)\, e^{i\omega t}d\omega.
\label{9.62}
\end{equation}
...
ima Fouriereva amplituda $\tilde{a}$ krajevno odvisnost $\exp(i\beta(\omega)z)$,
zato zadošča enačbi 
\begin{equation}
\frac{\partial \tilde{a}\left(z,\omega\right)}{\partial z}=i\,\beta(\omega) \,\tilde{a}(z,
\omega).
\label{9.63}
\end{equation}
Privzeli smo, da je spekter sunka ozek, zato lahko $\beta(\omega)$
razvijemo okoli $\omega_{0}$ in dobimo
\begin{equation}
\frac{\partial \tilde{a}\left(z,\omega\right)}{\partial z}=i\,\left(\beta(\omega_{0})
+\frac{d\beta}{d\omega}\,(\omega-\omega_{0})+\frac{1}{2}\,\frac{d^{2}\beta}{d\omega^{2}}\,(\omega-\omega_{0})^{2}\right)\, \tilde{a}.
\label{9.64}
\end{equation}
Vpeljimo novo amplitudno funkcijo $\tilde{A}$, ki ne vsebuje osnovne odvisnosti
$\exp(i\beta(\omega_{0})z)$ 
\begin{equation}
\tilde{a}\left(z,\omega\right)=\tilde{A}\left(z,\omega-\omega_{0}\right)\, e^{i\,\beta\left(\omega_{0}\right)\, z}.
\label{9.65}
\end{equation}
Ker je spekter različen od nič le okoli $\omega_{0}$, je prikladno
$\tilde{A}$ pisati kot funkcijo razlike $\omega-\omega_{0}$. Napravimo obratno Fourierevo
transformacijo zadnjega izraza (enačba~\ref{9.62})
\begin{eqnarray}
a\left(z,t\right) & = & \int_{-\infty}^{\infty}\tilde{A}\left(z,\omega-\omega_{0}\right)\,
e^{i(\beta(\omega_{0})\, z+\omega t)}\, d\omega
\label{9.66}\\
 & = & \int_{-\infty}^{\infty}\tilde{A}\left(z,\omega-\omega_{0}\right)\, 
 e^{i(\omega-\omega_{0})\, t} e^{i(\beta(\omega_{0})
 z+\omega_{0}t)} d(\omega-\omega_{0})\nonumber \\
 & = & A\left(z,t\right)\, e^{i (\beta(\omega_{0})\, z+\omega_{0}\, t)}.\nonumber 
\end{eqnarray}
Funkcija $A\left(z,t\right)$ očitno predstavlja prostorsko in časovno
odvisnost ovojnice sunka. Vstavimo definicijo (enačba~\ref{9.65}) v enačbo
(\ref{9.64}) 
\begin{equation}
\frac{\partial \tilde{A}(z,\omega-\omega_{0})}{\partial z}=i\left(\frac{d\beta}{d\omega}\,\left(\omega-\omega_{0}\right)+\frac{1}{2}\,\frac{d^{2}\beta}{d\omega^{2}}\,\left(\omega-\omega_{0}\right)^{2}\right)\, \tilde{A}\left(z,\omega-\omega_{0}\right).
\label{9.67}
\end{equation}
Z obratno Fourierevo transformacijo dobimo 
\begin{equation}
...
\end{equation}
 in vpeljali grupno hitrost kot $v_g = d\omega/d\beta$.
...
omenimo čelno sklopitev, sklopitev s prizmo, sklopitev preko uklonskih rež ali 
...
rod drugega. Povsem podobno pristopamo tudi pri izračunu sklopitve svetlobe v optično vlakno. Izračunati moramo, koliko vpadne svetlobe $E(r, \varphi)$ se sklopi z izbranim rodom vlakna $n,m$
\beq
...
z Gaussovim profilom z efektivnim polmerom. 

...
do izgub. Na tipičnem spoju vlaken pride tako do okoli $0,2-0,5$~dB izgub.

Omejimo se na primer dveh planarnih valovodov, ki sta tako tanki, da se po njima
širi le osnovni rod. Za primer obravnavajmo valovoda z različnima debelinama sredice,
ostali parametri pa naj bodo enaki. Zapišimo vpadno električno poljsko jakost 
kot (enačba~\ref{9.2})
\beq
{\mathbf E}_v(x,z)=\mathbf{e}\psi_v\left(x\right)\, e^{i\beta_v z},
\eeq
pri čemer prečno odvisnost zapišemo v $\psi$. Podobno lahko zapišemo tudi odbiti
in prepuščeni val. Privzemimo, da sta si vodnika razmeroma podobna in da se le majhen
delež moči izgublja preko plašča v okolico. Potem lahko za električno magnetno jakost
zapišemo 
\beq
\psi_v + r\psi_o = t \psi_p,
\eeq
pri čemer je $r$ koeficient odbojnosti, $t$ koeficient prepustnosti, $\psi_o$ in $\psi_p$
pa opisujeta odbiti in prepuščeni žarek. 

...
Povprečno energijo izbranega lastnega stanja zapišemo kot produkt energije fotona in 
...
stanja $\langle W \rangle$ pomnožimo z gostoto stanj $\varrho (\omega)$ 
...
preko celotne spektralne širine prehoda $g(\omega - \omega_0)$ približno konstantna 
...
Vpeljemo še simbol za matrični element koordinate med 
atomskima stanjema
...
\begin{equation}
\frac{d c_1}{dt}=-\frac{i}{\hbar} \mathcal{V} \cos (\omega t) e^{-i\omega_0 t}\, c_2 
\qquad \mathrm{in} \qquad
\frac{d c_2}{dt}=-\frac{i}{\hbar} \mathcal{V} \cos (\omega t) e^{i\omega_0 t}\, c_1,
\label{eq:c1c2}
\end{equation}
pri čemer je $\mathcal{V} = -\langle1|e\hat{x}E_0|2\rangle$. Zapišemo $\cos(\omega t)$ kot
kompleksno število in zanemarimo hitro spreminjajočo se komponento pri $\omega_0 + \omega$,
tako da enačbi prepišemo v 
\begin{equation}
\frac{d c_1}{dt}=-\frac{i}{2\hbar} \mathcal{V} e^{-i\Delta t}\, c_2 
\qquad \mathrm{in} \qquad
\frac{d c_2}{dt}=-\frac{i}{2\hbar} \mathcal{V} e^{i\Delta t}\, c_1,
\label{eq:rabi2}
\end{equation}
...
\label{eq:rabi3} \\
c_2(t)&=&\frac{\mathcal{V}}{i\hbar \Omega} e^{i\Delta t/2}\, \sin\left(\frac{\Omega t}{2}\right).
\label{eq:rabi4}
\end{eqnarray}
Pri tem smo vpeljali krožno frekvenco
\begin{equation}
\Omega = \sqrt{\Delta^2 + \left(\frac{\mathcal{V}}{\hbar}\right)^2} = \sqrt{(\omega_0-\omega)^2 
+ \left(\frac{\langle1|\hat{x}|2\rangle\, eE_0}{\hbar}\right)^2} = 
\sqrt{(\omega_0-\omega)^2 + \Omega_R^2}.
\end{equation}
Krožno frekvenco $\Omega_R = \mathcal{V}/\hbar$ imenujemo Rabijeva krožna frekvenca.\index{Rabijeva frekvenca}
...
\begin{equation}
P_2(t) = |c_2(t)|^2 = \frac{\mathcal{V}^2}{\hbar^2 \Omega^2}\sin^2(\Omega t/2).
\end{equation}
Če je frekvenca vpadne svetlobe točno enaka frekvenci prehoda, je $\Delta = 0$ in 
$\Omega = \Omega_R = \mathcal{V}/\hbar$. Takrat je amplituda nihanja zasedenosti vzbujenega stanja kar enaka 1

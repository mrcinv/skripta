...
je poglavitni vzrok za razširitev črte Dopplerjev pojav.\index{Energijski nivoji!Argon}
...
tako izberemo valovno dolžino ojačane svetlobe. Nekaj tipičnih podatkov za argonski
...
V kombinaciji s kriptonovim laserjem, ki je zelo podobem argonskemu, le da deluje
...
(slika~\ref{fig:CO2}\,a). 
Za molekule tale oblike obstajajo trije osnovni načini nihanja atomov glede na težišče:
...
Posamezno vibracijsko stanje lahko razstavimo na osnovne nihajne načine in 
...
osnovni simetričen razteg, stanje 010 osnovno upogibno nihanje, stanje 001 pa 
...
Za delovanje mora biti obrnjena zasedenost razmeroma velika,
vendar se v sunku sprosti veliko energije. Da nastanejo ekscimeri, vzbujamo mešanico 
...
Začeli bomo z opisom dveh primerov neodimovega laserja, Nd:YAG in Nd:steklo. Podobne laserje dobimo, če namesto neodima v YAG kristalu itrijeve ione nadomestimo z iterbijem ($1030~\si{\nano\metre}$) ali erbijem ($2940~\si{\nano\metre}$).
...
načinu pri močeh $5-100~\si{\watt}$ ali sunkovnem z energijo sunka $\sim 1~\si{\joule}$.
...
$10~\si{\centi\metre}$ in širine okoli $1~\si{\centi\metre}$. 
V kristalu YAG neodimovi ioni nadomestijo približno $1~\%$ itrijevih, zato ojačevalno
sredstvo na videz ni več prozorno, temveč rahlo rožnato (slika~\ref{fig:Nd}\,a). 
Za črpanje uporabljamo diodne laserje ali močne ksenonove svetilke za zvezno delovanje 
ter podobne bliskovne luči za sunkovno delovanje. Skupaj z aktivno paličico so svetilke vgrajene
v cilindrično ali eliptično votlino z zrcalnimi ali belimi stenami, tako da se
čim večji del črpalne svetlobe absorbira v laserski paličici (slika~\ref{fig:Nd}\,b).

\begin{figure}[h]
\centering
\def\svgwidth{120truemm} 
\input{slike/07_Nd.pdf_tex}
\caption{Ojačevalno sredstvo v Nd:YAG laserju in shema eliptične črpalne votline}
\label{fig:Nd}
\end{figure}
...
za medicinske namene je tudi Fotona iz Ljubljane. 
...
manjšega ojačanja pri dani obrnjeni zasedenosti je v laserju s preklopom
dobrote možno doseči večjo načrpanost, ne da bi prišlo do praznjenja
...
okoli $5 \times 10^{11}$~V/m$^2$, kar je približno enako polju v vodikovem atomu.
\end{remark}

\section{Titan-safirni laser}
\index{Laser!titan-safirni}
...
\index{Energijski nivoji!titan-safir}
\begin{figure}[h]
\centering
\def\svgwidth{100truemm} 
...
frekvencam od absorpcijske. Absorpcijski in fluoresecenčni spekter prehoda $S_0-S_1$
...
\caption{Absorpcijski in emisijski sprekter barvila rodamin 6G, ki se uporablja v laserjih}
\label{fig:RhG}
\end{figure} 
...
Gostota obrnjene zasedenosti $\Delta N/V$ & $ \sim 10^{25}/\si{\metre}^3$ \\ \hline
\end{tabular}

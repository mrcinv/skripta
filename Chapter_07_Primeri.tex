...
dimenzija ($\sim 10$--$100~\si{\micro\metre}$), utečena izdelava in nizka cena, 
velik izkoristek ($\sim 50~\%$), predvsem pa neposredno črpanje z električnim tokom. 
Za črpanje zadoščajo že majhni tokovi 
(tipično $\sim 100~\si{\milli\ampere}$), kar omogoča zelo hitro modulacijo ($\si{GHz}$)
...
\begin{equation}  
\frac{1}{e^{(E_b-F_{p})}+1}>\frac{1}{e^{(E_a-F_v)}+1}.
\label{6.12}
\end{equation}
...
energije reže, sicer ne pride niti do absoprcije niti do ojačenja. Gornja zveza pa pove tudi, 
...
Svetloba se bo ojačala torej le v primeru, da je začetna energija manjša od $F_p$ in končna
večja od $F_v$.

Koeficient ojačenja vpadne svetlobe pri dani 
frekvenci $\omega$ je
\beq
...
temperaturo. Njegovo odvisnost od energije vpadnih fotonov kaže slika~(\ref{s6.11}).
\begin{figure}[h]
\centering
\def\svgwidth{80truemm} 
...
Najpomembnejši primer je plast $p$-GaAs, ki se nahaja med plastjo  $n$-Al$_x$Ga$_{1-x}$As in $p$-Al$_x$Ga$_{1-x}$As
...
Heterostruktura ima nekaj poglavitnih prednosti. Zlitina z aluminijem ima malenkost večjo 
...
poleg tega se zaradi večje energijske reže občuno zmanjša absorpcija izven aktivnega območja.
Skupaj z zmanjšano debelino aktivnega območja to vodi do nižjih tokov, 
potrebnih za delovanje laserja, izboljšanega izkoristka in zmanjšanja gretja.  
\begin{figure}[h]
\centering
\def\svgwidth{145truemm} 
\input{slike/07_hetero.pdf_tex}
\caption{Shema dvojne heterostrukture (levo) oblika energijskih pasov 
...
kakih $10~\si{\micro\meter}$ širok pas. Odjedkani material nadomestijo s čistim Al$_x$Ga$_{1-x}$As, 
...
premerom okoli $1~\si{\micro\meter}$ v navpični in $10~\si{\micro\meter}$ v prečni smeri. 
To da v večji oddaljenosti snop z divergenco kakih $50~\si{deg}$ v navpični in okoli $5~\si{\deg}$ 
v prečni smeri. Če potrebujemo cilindrično simetričen snop, ga moramo popraviti z ustreznimi 
cilindričnimi lečami (Naloga, SLIKA).
...
tanke aktivne plasti, vzporedno z ravnino spoja $p$-$n$, ampak izhaja iz večje površine. To so 
tako imenovani VCSEL ({\it Vertical cavity surface-emitting laser}). Ker je sevalna površina večja in 
ojačevalno sredstvo krajše, je ojačenje na prelet manjše in mora biti odbojnost resonatorja toliko večja. 
\end{remark}

\begin{remark}
Posebna vrsta polprevodniških laserjev so nizko-dimenzionalni laserji. Zaradi velikosti, ki
je vsaj v eni smeri primerljiva z medatomsko razdaljo (tipično pod $20~\si{nm}$), 
se bistveno zmanjša gostota stanj. Za dosego zahtevane razlike med kvazi-Fermijevih 
nivojev -- in s tem doseči prag za delovanje laserja --
je tako potrebna precej manjša gostota nosilcev naboja.
...
kvantne pike, je izsevana svetloba iz skupine kvantnih pik zelo nehomogeno razširjena. 
\end{remark}

Diodni laserji so danes najbolj razširjeni. Uporabljamo jih v vsakodnevnih napravah, pogosto 
pa jih uporabljamo tudi za črpanje trdninskih ali vlakenskih laserjev. Med drugim jih odlikuje tudi
zelo dolg življenjski čas (do 50 000 ur). S spreminjanjem črpalnega toka lahko delujejo sunkovno
s frekvencami več $\si{GHz}$, lahko pa delujejo tudi v načinu uklenjenih faz. 
...
Prve svetlobne diode so bile izdelane iz GaAsP in so oddajale svetlobo pri $710~\si{nm}$. Pomemben
napredek je bila izdelava svetlobnih diod, ki so oddajale svetlobo v modrem delu 
spektra.\footnote{Za iznajdbo modrih LED so Isamu Akasaki, Hiroshi Amano and 
...
ki oddaja svetlobo v modrem delu spektra, ki pajo prevlečemo z molekulami fosforja. 
...
Svetlobne diode odlikuje dolga življenjska doba (do 50 000 ur), dober izkoristek 
(nad $25~\%$). 

...
\end{equation}
Dolžina resonatorja $L$ je velika v primerjavi z $\lambda$, zato je $n$
...
\Delta\omega_n=\frac{\pi c}{L} \quad \mathrm{oziroma} \quad \Delta \nu_n = \frac{c}{2L}.
}
...
in resonator je stabilen pri $F>1$. Laserji imajo pogosto $F\gg 1$, lahko tudi $F \sim 100$.
...
odbojnostjo ${\cal R}$, lahko izračunamo (glej nalogo~\ref{naloga:FP}). Prepustnost je enaka 
\begin{equation}
...
enačbo (enačba~\ref{eq:obosna-valovna-enacba}). Stoječe valovanje se pojavi, kadar
...
radij čela snopa ujema s krivinskim radijem zrcal. Kadar $R_1 \neq R_2$, 
...
se taki resonatorji nahajajo na simetrali lihih kvadrantov pri $x=y$. V simetričnih resonatorjih 
...
Hiter račun pokaže, da velja $z_{0}=L/2$. Dolžina območja bližnjega polja je 
torej enaka dolžini resonatorja, snop pa se na razdalji od grla do zrcala razširi
za $w_1/w_0=\sqrt{2}$. 
\begin{definition}
...
\arctan\left(\frac{z_{1}}{z_{0}}\right)\right)=2n\pi.
\end{equation}
Resonančne frekvence so torej odvisne od števila prečnih vozlov (slika~\ref{fig:crte}),
kar je dodaten razlog, zakaj v laserjih vzbujamo le osnovno lastno nihanje.
...
simetričnem resonatorju je $z_{0}\gg L$. Takrat lahko $\arctan(L/2z_{0})$ razvijemo, 
...
Navadno je $F$ znatno večji od 1 in so uklonske izgube zanemarljive. 
\end{enumerate}

Celotne izgube popišemo z razpadnim časom za energijo 
\begin{equation}
...
od koder sledi
\begin{equation}
\frac{2}{\tau}=\frac{\Lambda c}{2L}=\frac{2}{\tau_{0}}+\frac{c}{2L}\left((1-{\cal R}_{1})
+(1-{\cal R}_{2})\right),
\label{taulambda}
\end{equation}
kjer smo s 
...
Notranje izgube $\Lambda_0$ so navadno zelo majhne $\Lambda_0 \sim 0$, 
...
pa je kot med zveznico in normalo na zrcali v osi. Vpeljemo faktor
\begin{equation}
...
\label{3.78}
\end{equation}
Povsem podobno napravimo tudi za $g(Y')$.
...
$\alpha = \sqrt{2\pi L/k}=\sqrt{\lambda L}$. Postavimo izračunano električno 
...
isti rezultat kot obosna valovna enačba. To nas seveda ne preseneča, saj je

...
izkoristek, kadar je $z_{1}=0,36\,L$, kar je malo manj kot pri
...
pojav imenujemo optično usmerjanje, saj iz svetlobnega valovanja dobimo statično polje. Tako statično 
...
Precej bolj uporaben pojav dobimo, če na nelinearen kristal posvetimo z ultrakratkimi 
...
namesto statičnega polja valovanje pri frekvencah, ki so podobne širini spektra. Ocenimo jih. 
...
\beq
P_{\mathrm{NL},x}=
\epsilon_{0} \left(\chi_{11} +\frac{3}{4} \chi_{1111}\frac{2 j }
{\varepsilon_0 \tilde{n} c_0}\right)E = \varepsilon_0 \chi_{ef}E = \varepsilon_0 (n^2-1) E.
\eeq
Ko izrazimo lomni količnik, dobimo
\beq
n \approx \tilde{n} + \frac{3 \chi_{1111}}{4 \varepsilon_0 c_0 \tilde{n}^2} j.
\eeq
...
Poglejmo še primer. V CS$_{2}$, ki je tekočina z razmeroma velikim optičnim
...
\beq
P_{s}=\frac{\lambda^2}{16 \pi n_2} = 6,2~\mathrm{kW}.
\eeq
...
Pojava optičnih solitonov ni težko razložiti. Naj na optično nelinearno sredstvo
...
\beq
\omega = \omega_0 + \frac{2k_0 n_2 z}{\tau^2} \, t \, e^{-t^2/\tau^2}.
\eeq
...
Za matematični opis optičnih solitonov izhajamo iz valovne enačbe (enačba~\ref{8.3}), ki
jo zapišemo v skalarni obliki
\begin{equation}
...
namesto $E$ in $P_{\mathrm{NL}}$ vpeljemo Fourierevi transformiranki $\tilde{E}$ in $\tilde{P}$ in dobimo
\begin{equation}
...
glede na valovno dolžino le počasi spreminja, zato drugi odvod zanemarimo in dobimo
\beq
...
razvijemo v Taylorjevo vrsto okoli osrednje frekvence $\omega_0$ do tretjega člena. Dobimo
\beq
...
funkcije $\tilde{A}(z,\Omega)$, $B(z,t)$ pa amplituda polarizacije in inverzna transformiranka 
...
\eeq
Gornja enačba ni nič drugega kot nelinearna Schr\"odingerjeva enačba, ki smo jo 
zapisali že pri izpeljavi krajevnih solitonov~(\ref{8.84}). Enačbi se razlikujeta v tem, da
ima vlogo prečne koorindate $x$ tukaj čas $\tau$ in rešitve nimajo več konstantnega premera,
ampak imajo konstantno dolžino sunka. Stacionarne reštive pa dobimo le v primeru, kadar je  $d^{2}k/d\omega^{2}<0$ oziroma kadar ima drugi odvod nasprotni predznak od nelinearnega lomnega količnika. 
Kot pri krajevnih solitonih vpeljemo parameter $\eta$, ki je sorazmeren 
...
\begin{remark}
Ena izmed snovi, ki izpoljnjuje pogoj, da je $k''$ nasprotnega predznaka kot $n_2$, so kvarčna 
optična vlakna. Pri valovnih dolžinah vidne svetlobe to sicer ne velja, velja pa za $\lambda > 1,3~\mu$m.
...
pojav, pri katerem dobimo iz danega valovanja novo valovanje, ki ima enake valovne
fronte, vendar potuje v nasprotni smeri od prvotnega valovanja. Novo valovanje je torej tako,
kot bi začetnemu valovanju obrnili preznak časa in ga ''zavrteli nazaj''. 
...
valovanju. Zaradi lastnosti, da lahko novo valovanje iz signalnega dobimo tako,
da krajevni del kompleksno konjugiramo, nastalemu valovanju pravimo fazno
konjugiran valovanje.

Uporabna posledica fazne konjugacije je prikazana na sliki~(\ref{08_OPC2}).
Najpreprostejši primer je vpad ravnega vala, ki se ne odbije po lomnem zakonu (slika levo),
ampak se odbije v smer, iz katere je vpadel na snov (desno). Drugi primer je krogelni vala
ali v približku tudi Gaussov snop. Ko vpade na navadno zrcalo (levo), se njegova divergenca
ohranja in se žarek še naprej razširja. Na fazno konjugiranem zrcalu se kroglast val spet
zbere v izvoru (desno). Tretji primer naj bo poljubno sredstvo, ki valovanju doda naključno
fazo, zato po prehodu valovne fronte niso več gladke. Ta popačen snop v faznem
...
in dobimo enake gladke valovne fronte ravnega vala, kot smo jih imeli na začetku. 
...
sredstvu vsota štirih valovanj, dveh črpalnih, signalnega in odbitega
\begin{equation}
...
Račun poenostavimo še z privzetkom, da sta črpalna vala $E_{1}$
...
enak $P_\mathrm{NL}= \epsilon_{0}\chi^{(3)}E^3$, pri čemer je $\chi^{(3)} = \chi$
efektivna nelinearna susceptibilnost
za izbrano polarizacijo vseh polj. Ker je $E$ zapisan kot vsota osmih različnih členov
(enačba~\ref{8.99}), vsebuje produkt $E^3$ kar 512 členov. Vendar se njihovo število znatno zmanjša, 
če upoštevamo le tiste z enako časovno odvisnostjo in zato z enako frekvenco.
...
k enačbi za $E_4$ pa tisti z $\exp(-ikz)$. Dobimo
\begin{eqnarray}
...
od intenzitete črpalnih valov, torej optični Kerrov pojav, in le dodaten prispevek
k fazi. Vpeljimo novi amplitudi, ki se od prejšnih razlikujeta zgolj v faznem faktorju.
\beq
\tilde{A}_3 = A_3 \exp\left(-i\frac{ 3 \chi \omega}{4 c_0 n}z\right)
\eeq
in 
\beq
\tilde{A}_4 = A_4 \exp\left(i\frac{ 3 \chi \omega}{4 c_0 n}z\right).
\eeq
Ko novi amplitudi vstavimo v diferencialni enačbi~(\ref{eq:opc1} in 
...
\textrm{in} \qquad \frac{d\tilde{A}_{4}^{*}}{dz}=i\kappa^* \tilde{A}_{3}.
\label{8.107a}
\eeq 
oziroma
\begin{equation}
...
\label{8.107}
\end{equation}
Tako smo zelo težaven problem nelinearne valovne enačbe poenostavili na linearen
sistem dveh preprostih sklopljenih enačb za amplitudi signalnega in
odbitega vala. Splošni rešitvi sistema enačb~(\ref{8.107}) 
sta 
\begin{eqnarray}
\tilde{A}_3^* \left(z\right) & = & C_{1}\cos\left|\kappa\right|z+
C_{2}\sin\left|\kappa\right|z
\label{8.108}\\
\tilde{A}_{4}\left(z\right) & = & -i\frac{\left|\kappa\right|}{\kappa^{*}}
\left(-C_{1}\sin\left|\kappa\right|z+C_{2}\cos\left|\kappa\right|z\right)\nonumber 
\end{eqnarray}
...
\begin{equation}
A_{4}\left(0\right)=i\frac{\kappa^{*}}{\left|\kappa\right|}\tan\left|\kappa\right|L\; A_{3}^{*}\left(0\right)
\label{8.110}
\end{equation}

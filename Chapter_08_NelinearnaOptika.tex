...
podvajanje frekvenc, optično parametrično ojačevanje, optično usmerjanje, 
...
splošnega tenzorja tretjega ranga smo tako uvedli matriko velikosti $3\times6$. 
...
so navedene eksperimentalno dobljene nelinearne susceptibilnosti\footnote{Izmerjene vrednosti, ki jih najdemo v literaturi, se od vira do vira pogosto znatno razlikujejo.} in vidimo, da Kleinmanova
domneva ni povsem točna, ampak je razmeroma dober približek. 
...
Tako dobimo poleg valovanj z osnovnima frekvencama tudi valovanje pri dvakratnikih
...
\begin{remark}
Nastanku valovanja pri podvojeni frekvenci oziroma optičnemu podvajanju frekvenc pravimo tudi
...
Ugotovili smo že, da pri velikih intenzitetah vpadnih valovanj navadna valovna 
enačba ne velja več. Pride do pojava
...
energijskim tokom pri podvojeni in osnovni frekvenci oziroma izkoristek pretvorbe enak
\boxeq{8.11}{
...
dolžine poti, izkoristek pretvorbe pa narašča sorazmerno s kvadratom dolžine
\beq
\frac{P_{2\omega}}{P_{\omega}}=
\frac{\omega^2 \chi_{ef}^2}{2 S n_{2\omega} n_\omega^2c_0^3\varepsilon_0} P_\omega L^2.
\eeq
...
v $k$ prostoru\index{Lomni količnik} za pozitivno
...
susceptibilnost $\chi_{ef}$, ki jo moramo izračunati za vsak primer posebej, odvisna 
pa je od polarizacij vhodnega in izhodnega žarka ter seveda simetrije kristala. Ugotovili smo že, 
...
Oglejmo si kot primer KH$_{2}$PO$_{4}$\index{KDP}, ki je negativno anizotropen 
...
Naša naloga je poiskati vrednost kota $\varphi$, pri kateri je 
...
\label{8.15}
\end{equation}
To najlažje preverimo, tako da postavimo vektorj $\mathbf{s}$ enkrat v ravnino $xz$ in
...
snopa po celotni površini. Sledi
\beq
S=\frac{1}{2}\pi w_{0}^{2} = \frac{2 \pi c_0 L}{n \omega}.
\eeq
...
\boxeq{8.17}{
\frac{P_{2\omega}}{P_{\omega}}=
\frac{\omega^3 \chi_{ef}^2}{\pi n_{2\omega} n_\omega c_0^4\varepsilon_0} P_\omega\, L.
}
Ob optimalnem fokusiranju je torej izkoristek pretvorbe sorazmeren z dolžino kristala.
\begin{definition}
...
\frac{2 \chi_{ef}^2 \omega^3 P_\omega z_1}{\pi c_0^4 \varepsilon_0 n_\omega n_{2\omega}} 
\arctan^2 \left( \frac{L}{2z_1}\right) \\
 = &\frac{2\chi_{ef}^2 \omega^3 P_\omega}{\pi c_0^4 \varepsilon_0 n_\omega n_{2\omega}} \frac{L}{2}
...
\begin{equation}
\frac{P_{2\omega}}{P_{\omega}}
= 0,64 \frac{\omega^3 \chi_{ef}^2}{\pi n_{2\omega} n_{\omega} c_0^4 \varepsilon_0 } P_\omega L.
\label{8.28}
\end{equation}
...
$n = 2,2$, efektivna nelinearna susceptibilnost pa  $\chi_{ef} = 5$~pm/V. Vstavimo podatke v enačbo~(\ref{opakapa}) in izračunamo vrednost
...
Za delovanje oscilatorja mora biti jakost črpalnega žarka tako velika, da je parametrično 
...
ojačuje le enkrat na celoten obhod. Ob preletu v drugo smer je namreč $\Delta k \neq 0$ in žarek se ne ojačuje. V staconarnem stanju je moč 
žarka po obhodu $P_2$ enaka začetni moči $P_0$, saj je pri pragu ojačanje ravno enako izgubam 
\beq
...
Vzemimo sunek svetlobe, ki je dolg 1~ps. Njegova frekvenčna širina je potem
\beq
...
spektra 
\beq
\Delta \omega = \frac{2}{10^{-12}~\mathrm{Hz}} = 2~\mathrm{THz}.
\eeq
...
učinek je mešanje treh frekvenc, na primer optično podvajanje frekvence ali

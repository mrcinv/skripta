...
\subsection{Optični Kerrov pojav}
Naj valovanje vpada na nelinearno snov, katere $\chi^{(2)} = 0$.
Polarizacija je potem enaka vsoti linearnega in nelinearnega dela (enačba~\ref{8.1})
\beq
\mathbf{P}=
\epsilon_{0} \chi^{(1)}\cdot \mathbf{E}+
...
\label{8.71}
\end{equation}
Zanimajo nas samo členi v polarizaciji, ki nihajo s frekvenco $\omega$. Te dobimo tako, da
v izrazu za $\mathbf{E}\mathbin \mathbf{E}\mathbin\mathbf{E}$ 
...
\beq
\mathbf{P}=
\epsilon_{0} \left( \chi^{(1)}\cdot +\frac{3}{4} \chi^{(3)}\vdots \mathbin \mathbf{e}\mathbin 
\mathbf{e} \frac{2 \langle j \rangle}{\varepsilon_0 n_\omega c_0}\right) \mathbf{E}.
\eeq
...
pri katerem vpadna svetloba vpliva na lomni količnik snovi. Ker gre za podoben učinek kot pri
Kerrovem pojavu, 
pri katerem se lomni količnik spremeni pod vplivom zunanjega električnega polja, 
imenujemo opisan pojav optični Kerrov pojav\footnote{Škotski fizik John Kerr, 1824\textendash1907.}. 

Poglejmo pojav na primeru izotropne snovi, na snov pa naj vpada valovanje, ki je polarizirano
v smeri $x$. Nelinearna polarizacija ima tako le komponento 
\beq
P_{x,\mathrm{NL}}=
\epsilon_{0} \left(\chi_{11}}\cdot +\frac{3}{4} \chi_{1111}\frac{2 \langle j \rangle}
{\varepsilon_0 n_\omega c_0}\right)E = \varepsilon_0 \chi_{ef}E = \varepsilon (\varepsilon-1) E.
\eeq

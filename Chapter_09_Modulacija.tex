...
\label{7.32}
\end{equation}
pri čemer smo uporabili Braggov pogoj (enačba~\ref{7.29}). Sprememba frekvence
...
Z vklapljanjem in izklaplanjem zvočnega vala, ki ga vzbujamo s piezoelektričnim elementom,
...
$\mathbf{k}=(k_{1},0,k_{3})= k(\sin\vartheta,0,\cos\vartheta)$.
...
\begin{equation}
\epsilon=\tilde{\epsilon}+\Delta\epsilon\sin(qx-\Omega t),
\label{7.33}
\end{equation}
kjer je sprememba $\Delta\epsilon$ povezana z amplitudo deformacije
$S_{0}$ v zvočnem valu~(enačba~\ref{7.29})
\beq 
\Delta\epsilon=-\tilde{\epsilon}^{2}pS_{0}.
\eeq Izpustili smo indekse tenzorjev.

Zapišimo zdaj valovno enačbo 
\begin{equation}
...
s $P_{1}$ je, da ravnemu valu z valovnim vektorjem $\mathbf{k}$ in
frekvenco $\omega$ primeša val z valovnim vektorjem $\mathbf{k}\pm\mathbf{q}$

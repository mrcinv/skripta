...
v nematični fazni orientacijsko ureditev dolgega dosega. Če nematik segrejemo,
...
\caption{Molekula enega najbolj razširjenih tekočih kristalov, 4-ciano-4'pentilbifenila 
ali 5CB (levo) in shematski prikaz nematične faze (desno).}
\label{fig:5CB}
\end{figure}

Molekule so v nematični fazi v povprečju orientacijsko urejene, zato se nematični tekoči kristal
obnaša kot enoosen dvolomni kristal\index{Dvolomnost}. Njegova optična os je vzporedna 
z $\mathbf{n}$, lastni vrednosti dielektričnega tenzorja pa sta $\varepsilon_{\parallel}$
in $\varepsilon_\bot$. 
...
V povprečju so molekule urejene v smeri direktorja. Če se lokalno smer direktorja
spremeni, je energija takega deformiranega 
stanja nekoliko večja od energije homogenega urejenega stanja in 
okolica na delček tekočega kristala deluje z navorom v smeri zmanjševanja 
...
po vzorcu neurejeno spreminja. V optičnih napravah pa potrebujemo urejene vzorce, zato
 ...
 ali pa vzorce pripravimo dovolj tanke, da ureditev vsilijo mejne površine. Poglejmo, kako.

Na ureditev molekul tekočega kristala vpliva zunanje električno ali magnento polje.
Zaradi urejenosti molekul električna (in magnetna) susceptibilnost nematskega tekočega
...
in $\mathbf{n}$. Gostoto električne energije zapišemo kot 
\begin{equation}
...
del dielektrične konstante. Anizotropni del energije je tako
\beq
...
zunanjega polja, če je $\epsilon_{a}<0$, pa pravokotno nanj.
...
plastmi ali mehansko obdelamo), se molekule tekočega kristala tik ob povšini uredijo.
Tako na primer podrgnjena tanka plast najlona uredi $\mathbf{n}$ ob površini v smeri
drgnenja vzporedno s površino. Da je $\mathbf{n}$ pravokoten na površino,
...
vzporedni ali pravokotni ureditvi ob steni, se urejenost zaradi orientacijske elastičnosti
...
vzporedno ureditev. Vzorec je urejen in homogen, optična os leži v ravnini 
plasti (slika~\ref{LCD1}). Če dodamo na površini še prozorni elektrodi, lahko
z zunanjo napetostjo spreminjamo orientacijo molekul v plasti in tako tudi 
smer optične osi. Dovolj velika napetost zasuče $\mathbf{n}$ in s tem optično os
pravokotno na stene, razen tik ob površini. Pri debelini okoli 10~$\mu$m
je potrebna napetost nekaj voltov.

Ta pojav lahko izkoristimo za prepost optični preklopnik. Naj debelina plasti $d$
ustreza debelini ploščice $\lambda/2$ za izbrano valovno dolžino svetlobe 
\begin{equation}
d(n_{e}-n_{o})=(2N+1)\frac{\lambda}{2},
\label{7.57}
\end{equation}
kjer je $n_e$ izredni lomni količnik, $n_o$ redni 
lomni količnik, $N$ pa celo število. Ker v nematikih $n_{e}-n_{o}\sim0,1$, 
je ustrezna debelina $d$ nekaj $\mu$m. Vzorec damo med dva prekrižana 
polarizatorja s prepustno smerjo pod kotom 45$^{o}$ glede na $\mathbf{n}$
oziroma optično os. Vzorec, ki deluje kot ploščica $\lambda/2$, 
polarizacijo svetlobe z izbrano valovno dolžino zasuče za 90$^{o}$ in 
svetloba prehaja tudi skozi analizator. Ko
priključimo napetost, se optična os obrne v smeri polja, 
polarizacija svetlobe se pri prehodu skozi plast ohrani in 
...
Tak preklopnik ima nekaj slabosti. Prepustnost je odvisna
...
Zasukan  nematik dobimo tako, da površini, ki vsiljujeta vzporedno ureditev,
zasučemo za kot 90$^{o}$ eno glede na drugo (slika~\ref{LDC2}). 
Tedaj se smer $\mathbf{n}$ v plasti zvezno zavrti od ene do druge površine.
V nadaljevanju bomo pokazali, da polarizacija svetlobe, ki je ob vstopu 
v plast polarizirana v smeri urejanja, pri prehodu skozi plast približno 
sledi smeri $\mathbf{n}$ in je zato ob izstopu iz plasti polarizirana pravokotno
na vpadno polarizacijo. Ko priključimo električno polje, spremenimo smer optične 
osi, ki se obrne pravokotno na plast tekočega kristala. V tem primeru 
se polarizacija ne zasuče in analizator svetlobe ne prepusti. Plast med prekrižanima
polarizatorjema brez polja torej prepušča svetlobo, v polju pa ne. Pri tem
...
kristalnem elektro-optičnem modulatorju smo nameč lahko dobili tudi vmesne
prepustnosti, medtem ko lahko s tekočimi kristali na opisan način
dosežemo le zaprto in odprto stanje.

\begin{remark}
 Kadar želimo, da kazalnik deluje v odbiti svetlobi,
damo za zadnji analizator še odbojno površino. Barve pikslov, preklop, hitrost...

\end{remark}

Pokazati moramo še, da polarizacija svetlobe približno sledi zasuku optične osi. 
Sredstvo naj bo lokalno enoosno, optična os pa se suče v smeri pravokotno
glede na smer širjenja svetlobe. Poleg zasukane nematske celice je pomemben primer
snovi s takimi lastnostmi holesteričen tekoči kristal, ki je zelo
podoben nematskim, le da se $\mathbf{n}$ spontano suče okoli
...
\begin{definition}
Pokaži, da se dielektrični tenzor v zasukani nematski plasti zapiše kot
\begin{equation}
...
\label{7.59}
\end{equation}
kjer  je $z$ razdalja od spodnje plasti, v kateri je direktor
...
hitro uvidimo, da je valovna enačba, ki jo rešujemo, 
...
za razlago delovanja zasukane nematske celice zadošča približek 
...
in redni val v navadnem enoosnem kristalu. Vstavimo ju v eno od enačb~(\ref{7.65a}) ali
...
nematsko celico vpada svetloba, ki je polarizirana ali paralelno ali pravokotno
...
frekvenčnem območju ima zveza štiri rešitve za vsako frekvenco $(\beta \propto \omega)$. }
\label{gap}
\end{figure}
\end{remark}
...
\label{7.76}
\end{equation}
Prosta energija je tako 
\begin{eqnarray}
F & = & \frac{1}{2}\int\left[K_{1}\left(\frac{dn_{z}}{dz}\right)^{2}+K_{2}n_{z}^{2}\left(\frac{dn_{z}}{dz}\right)^{2}-
\epsilon_{0}\epsilon_{a}(n_{z}E)^{2}\right]dz=\nonumber \\
 & = & \frac{1}{2}\int_{0}^{d}
 [K_{1}q^{2}a^{2}\cos^{2}qz+K_{3}q^{2}a^{4}\sin^{2}qz\cos^{2}qz-
 \epsilon_{0}\epsilon_{a}Ea^{2}\sin^{2}qz]dz=\nonumber \\
 & = & \frac{\pi}{4q}[K_{1}q^{2}a^{2}+\frac{1}
 {4}K_{3}q^{2}a^{4}-\epsilon_{0}\epsilon_{a}Ea^{2}]\;.
\end{eqnarray}

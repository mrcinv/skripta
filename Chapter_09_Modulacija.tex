...
polje $E_z$, se tenzor dielektričnosti zaradi Pockelsovega pojava spremeni. Sprememba
inverznega tenzorja je po enačbi~(\ref{eq:Pockels})
\begin{align}
...
pri kateri je \index{$\pi$-napetost}
...
n_z' &\approx n_e.
\end{align}
Kako pa je z novimi lastnimi osmi?\index{Dvolomnost!dvoosne snovi} Hitro ugotovimo (enačba~\ref{7.8b}), da se ob
...
Karakteristična $\pi$-napetost \index{$\pi$-napetost}je 
\boxeq{UpiT}{
...
velja $p_{11} \simeq p_{12} = 0,31$ in $p_{44} \sim 0$, za LiNbO$_3$\index{LiNbO$_3$} pa $p_{11} = -0,02, 
...
\caption{Ob vpadu svetlobe na tanko plast, v kateri je vzbujeno zvočno valovanje 
(a), se pojavi veliko uklonskih vrhov, 
na debeli plasti (b) pa je opazen zgolj en uklonjen vrh, 

...
pa Kerrov tenzor\index{Kerrov tenzor}. 
...
\end{align}
Os $z$ se ohranja, drugi dve novi lastni osi pa sta zasukani za kot $45~^\circ$ glede na prvotni osi $x$ in $y$.
...
polja je kristal enoosen z optično osjo v smeri $z$. Lomni količnik je neodvisen od
polarizacije vpadnega valovanja in je enak $n_o$. Ko priključimo polje, postane kristal
optično dvoosen, saj so vse tri lastne vrednosti tenzorja dielektričnosti različne. \index{Dvolomnost!Dvoosne snovi}
...
Za kristal KDP je $\pi$-vrednost napetosti pri valovni dolžini $633~\si{\nano\metre}$ okoli  $9000~\si{\volt}$, kar 
...
Če privzamemo, da je sprememba zaradi električnega polja majhna ($rE\ll1$),
...
Včasih pa želimo modulirati fazo vpadne svetlobe. Tudi to si oglejmo na primeru longitudinalne
 modulacije\index{Elektro-optična modulacija!longitudinalna}. Fazno oziroma frekvenčno modulacijo dosežemo tako,
...
Takrat modulator deluje kot tanka uklonska mrežica \index{Uklon}in pojavi se veliko 
...
\boxeq{R-N}{
\Lambda (\sin \vartheta - \sin \beta ) = \pm N \lambda,
}
...
(pod $\sim10$~MHz) in majhnih debelinah (pod $\sim 1$~cm) pri poljubnem vpadnem 
kotu $\vartheta$.
\begin{figure}[h]
\centering
...
dobrote laserskega resonatorja.
...
obnaša kot enoosen dvolomni kristal\index{Dvolomnost!Enoosna snov}. Njegova optična os je vzporedna 
...
\begin{figure}[h!]
\centering
\includegraphics[width=70truemm]{slike/09_LCD.jpg}

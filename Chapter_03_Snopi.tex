...
V veliki oddaljenosti od zaslona polje lahko izračunamo s
...
kar pa ni najudobnejša pot. Lotimo se naloge raje z uporabo približka
...
Obravnavo začnemo z valovno enačbo in monokromatskim valovanjem s frekvenco $\omega$. Ustrezna
...
\end{equation}
Očitno sledi enačba~(\ref{eq:k3-razvoj}) iz enačbe~(\ref{eq:k3-tocno})
...
\end{equation}
Ta zveza mora veljati pri vsakem $r$, zato so koeficienti
...
$z_{0}$ je razdalja, pri kateri preide snop v asimptotično enakomerno širjenje in določa
...
snopa v odvisnosti od oddaljenosti od grla $z$. To najlažje
...
toka ob širjenju žarka, poleg tega pa da še dodatno spremembo faze. Zapišemo ga v obliki
...
Pokaži, da celotna svetlobna moč v snopu enaka $P = \int I dS = \frac{1}{2}\pi w_0^2 I_0$.
\end{definition}
...
Izkazalo se bo, da je kompleksni krivinski radij zelo uporaben pri obravnavi preslikav 
Gaussovih snopov čez leče.

Primerjajmo še Gaussov snop z drugimi valovanji. Na sliki (\ref{fig:ravni-Gaussov-krogelni-val}) 
so shematsko prikazane valovne fronte ravnega vala, Gaussovega snopa (enačba~\ref{eq:gaussov-snop}) 
in krogelnega vala (enačba~\ref{eq:krogelni-val}). Vidimo, da je za majhne oddaljenosti od grla $z$ Gaussov
...
je Gaussov snop (enačba~\ref{eq:gaussov-snop}). Poleg te rešitve pa obstaja še veliko 
...
Namig: pri zapisu prečne odvisnosti polja upoštevaj le vodilni člen
 Hermitovih polinomov in določi, na kateri razdalji od središča snopa ima polje $\psi$
največjo amplitudo.
\end{definition}
...
To je v skladu s tem, da je Fouriereva transformacija Hermite-Gaussove funkcije 
...
\caption{Prečni profil električne poljske jakosti Laguerre-Gaussovih snopov v grlu za
...
Kot nastavek za eksaktno rešitev valovne enačbe (enačba~\ref{eq:valovna-skalarna}), 
pri čemer obravnavamo polje skalarno, izberemo
\begin{equation}
E=E_{0}\psi(x,y)e^{i\beta z-i\omega t},
\end{equation}
kjer mora nastavek $\psi$ zadostovati Helmholtzevi enačbi\index{Helmholtzeva enačba}
\begin{equation}
...
\begin{equation}
\psi_m(x,y)=A_{m}J_{m}(k_{\perp}r)e^{im\varphi},
\end{equation}
kjer je $J_{m}$ Besslova funkcija in $m=0,\pm1,\pm2 \ldots$ Za
$m=0$ ima val obliko
\begin{equation}
E(r,z,t)=A_{0}J_{0}(k_{\perp}r)e^{i\beta z-i \omega t},
...
omejen v pravem smislu. Za velike oddaljenosti od središča snopa $r$ intenzitetni profil 
...
skozi optične naprave\index{Preslikava čez lečo}. Začnemo
...
\end{equation}
Kompleksni krivinski radij $q$ pa je po enačbi~(\ref{eq:q}) linearna funkcija koordinate $z$.
...
in določa lego grla preslikanega snopa na desni strani leče, druga pa povečavo
...
velja ne prva ne druga limita, temveč je treba uporabiti izračunani izraz (enačba~\ref{eq:preslikava-grlo}).
...
\end{equation}
Če torej želimo doseči čim manjše grlo po prehodu skozi lečo, mora biti polmer
vpadnega žarka čim večji. Vpadni žarek je tako smiselno
...
\end{array}\right].
\end{equation}
Preslikavo snopa zapišemo kot matriko, ki deluje na vpadni vektor in ga preslika
...
z matrikami. Optični element, ki ga prestavlja matrika $M$, preslika žarek $(y_{1},\vartheta_{1})$
v $(y_{2},\vartheta_{2})$.}
\label{fig:K-matricni-obravnavi}
\end{figure}

Matrike $M$ so v splošnem oblike
\begin{equation}
...
\end{array}\right],
\end{equation}
zato jih imenujemo tudi ABCD matrike\index{ABCD matrike}. Poglejmo nekaj primerov. 
...
\end{array}\right].
\end{equation}
Podobno izpeljavo kot za prehod skozi lečo lahko naredimo za odboj na sferičnem zrcalu
...
\end{eqnarray}
\begin{remark}
Opisan matrični formalizem je zelo prikladen predvsem za računanje prehoda
...
Ko koeficiente preslikave razvrstimo v ABCD matriko 
\begin{equation}
M= \left[\begin{array}{cc}
A & B\\
C & D
\end{array}\right].
\end{equation}
in iz gornjih enačb razberemo koeficiente ABCD matrik\index{ABCD matrike} za opisane 
preslikave, vidimo, da so povsem enaki kot v primeru geometrijske optike. Hitro lahko tudi preverimo, 

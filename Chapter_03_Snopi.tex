...
njegovo osnovno rešitev: Gaussov snop. Obravnavali bomo tudi snope višjega reda in
...
in prečno odvisnost zapišemo kot $\exp(-r^2/w^2)$. Gaussova oblika prečne odvisnosti da snopu tudi ime. Parameter $w$, ki označuje 
...
\end{equation}
Vpeljemo še $w_0 = 2z_0/k$ kot polmer snopa v izhodišču in zapišemo
\boxeq{eq:w}{
w^2 = w_{0}^{2}\left(1+\left(\frac{z}{z_{0}}\right)^{2}\right).
}
Pri $z=0$ je polmer snopa najmanjši in pravimo, da je tam grlo snopa\index{Gaussov snop!grlo}. Zapišemo zvezo
\boxeq{eq:z0}{
z_{0}=\frac{\pi w_{0}^{2}}{\lambda}.
}
Parameter $z_{0}$\index{Gaussov snop!bližnje polje} označuje oddaljenost od grla, 
pri kateri snop preide v asimptotično enakomerno širjenje. Parameter $z_0$ tako omejuje območje,
...
območje\index{Rayleighovo območje|see{Območje bližnjega polja}}. Celotna dolžina 
območja bližnjega polja je zaradi simetričnosti enaka $2z_0$. 
...
\end{equation}
Krivinski radij je v izhodišču neskončen, kar pomeni, da so tam valovne fronte Gaussovega
snopa ravne. Za velike oddaljenosti krivinski radij narašča linearno z oddaljenostjo in fronte
...
Ostane še faktor pred eksponentom v izrazu~(\ref{eq:gaussov-snop-vmesni})
\begin{equation}
...
pri čemer so $w=w(z)$, $R=R(z)$ in $\eta = \eta(z)$.
...
$632,8~\si{\nano\metre}$. Grlo snopa, ki naj leži na izhodu iz laserja, naj ima
polmer $w_{01}=0,5~\si{\milli\metre}$. Za tak snop je $z_{01}=124~\si{\centi\metre}$. 

...
in $\mathbf{B}$. V splošnem sta njuni odvisnosti zelo zapleteni,
...
enačbo\index{Valovna enačba} za jakost električnega in gostoto magnetnega polja 
...
\index{Poyntingov teorem}Poyntingov teorem, ki ga lahko izpeljemo neposredno 
iz Maxwellovih enačb in konsitutivnih relacij, predstavlja izrek o ohranitvi 
...
Vektorja jakosti električnega $\mathbf{E}$ in magnetnega polja $\mathbf{H}$ zadostujeta
Maxwellovim enačbam (enačbe~\ref{eq:Maxwell1}--\ref{eq:Maxwell4}), iz katerih sledi,
da sta polji vedno medsebojno pravokotni, hkrati pa v izotropnih snoveh pravokotni 
na valovni vektor $\mathbf{k}$ (naloga~\ref{naloga-TEM-ortogonalnost}).
Elektromagnetno valovanje je torej transverzalno valovanje. 
...
Pokaži, da za ravni val vedno velja 
\begin{equation}
\mathbf{D} \perp \mathbf{k} \quad \mathrm{in} \quad \mathbf{B} \perp \mathbf{k}.  
\end{equation}
Izpelji še zvezi
\begin{align}
...
\end{align}
pri čemer so komponente $A_{ij}$ odvisne od lastnosti sredstva. 
Enačbi zapišemo 
...
Za TE polarizacijo ju zapišemo 
\begin{equation}
...
Vidimo, da je pri nekem kotu, imenujemo ga \index{Brewstrov kot}Brewstrov 
...
Brewstrova okna\index{Brewstrovo okno}. Njihova značilnost je, da eno polarizacijo (TM) prepustijo v celoti, druge 
...
vse pogosteje uporabljajo tudi v bližnjem infrardečem področju svetlobe.\index{Infrardeče valovanje}
\end{remark}
...
razdaljo $r$ s koordinatama na zaslonu $x'$ in $y'$ ter
...
čemer $r$ v imenovalcu nadomestimo kar z $z$. Dobljen zapis je Fresnelov uklonski
približek 
\begin{equation}
\label{eq:FresnelApprox}
E_P(x,y,z) =  \frac{1}{i \lambda z } e^{i k z}\int \int E_S\, e^{ik ((x-x')^2+(y-y')^2)/2z} dx' dy'.
\end{equation}
Kadar zadošča razvoj le do linearnih členov, govorimo o Fraunhoferjevem
uklonu. Fraunhoferjev uklonski integral zapišemo kot
\begin{equation}
\label{eq:FraunhoferApprox}
E_P(x,y,z) =  \frac{1}{i\lambda z} e^{i k (z + (x^2+y^2) /2z)}\int \int E_S\,
e^{-ik (xx'+yy')/z} dx' dy'
\end{equation}
in v njem  prepoznamo Fourierevo transformacijo polja $E_S$.
...
\caption{Skicirana značilna uklonska slika odprtine v Fraunhoferjevem režimu (a), 
ko je oodaljenost od zaslona bistveno večja od velikosti odprtine, in v Fresnelovem režimu (b).}
\label{fig:UklonFF}
\end{figure}
...
\end{equation}
kjer je $a$ premer, $J_1(x)$ je Besslova funkcija in $\rho = \sqrt{x^2+y^2}$.
\end{definition}
...
pa ustrezajo trem lomnim količnikom\index{Lomni količnik} $\epsilon_{i}=n_{i}^2$. Snovi,
...
Ugotovili smo, da je v anizotropnih snoveh lomni količnik odvisen od smeri 
razširjanja svetlobe in izkaže se, da tudi od njene polarizacije. Poglejmo 
...
$y$, za katerega je lomni količnik enak $n_{2}$. Če pa polarizacija valovanja, 
ki se širi vzdolž lastne osi $z$, ne sovpada z lastnima osema $x$ ali $y$, nastane po 
prehodu skozi kristal iz vpadnega linearno polariziranega valovanja v splošnem eliptično valovanje\index{Polarizacija!eliptična}. Lastni komponenti namreč potujeta različno
hitro, zato pride med njima do faznega zamika. Za poljubno smer širjenja valovanja 
ter poljubno polarizacijo je račun razmeroma zapleten. 
...
% elipsoida sovpadajo z glavnimi osmi tenzorja $\underline{\varepsilon}$, polosi elipsoida pa so enake
...
\begin{remark}
Formalen pristop izhaja iz valovne enačbe
...
vodi pri vsakem valovnem vektorju do dveh rešitev in dveh različnih lomnih
 količnikov, od katerih vsak ustreza eni od ortogonalnih polarizacij. 
 V štirih točkah, vse ležijo v eni ravnini, 
 ...
 optični osi in tak kristal imenujemo optično dvoosen. Kadar se ploskev dotika sama sebe le v dveh 
 točkah, po dogovoru ležita ti dve točki na osi $z$, govorimo o optično enoosnem kristalu. 
\end{remark}

\subsection*{Optično enoosni kristali}
\index{Dvolomnost!enoosne snovi}
V optično enoosnih kristalih sta dve lastni vrednosti enaki. 


Po dogovoru izberemo lastne vrednosti tako, da velja $n_{1}=n_{2}\neq n_{3}$. 
...
\begin{tabular}{|l|c|c|} \hline  
      ...
      KH$_2$PO$_4$ & 1,5074 & 1,4669 \\ \hline \index{KH$_2$PO$_4$|see {KDP}}\index{KDP}

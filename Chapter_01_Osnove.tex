...
Če vstavimo nastavek za monokromatski val (enačbi~\ref{eq:rval}) v valovno enačbo
...
Vpeljemo lahko tudi kompleksni Poyntingov vektor\index{Poyntingov vektor}
\begin{equation}
...
\frac{1}{4}\frac{\left|H_{0}\right|^{2}}{\mu\mu_{0}}=\frac{1}{2}\epsilon\epsilon_{0}\left|E_{0}\right|^{2}.
\end{equation}
...
sredstvu vedno pravokotna na smer valovnega vektorja\footnote{V splošnem 
...
opisujeta polarizaciji vstopnega in izstopnega vala, $A$ pa je \index{Jonesova 
matrika}Jonesova matrika. Jonesova matrika za prehod skozi linearni polarizator, ki
...
(tak element imenujemo ploščica $\lambda/2$\index{Ploščica $\lambda/2$}), je
\begin{equation}
A=\left[\begin{array}{cc}
1 & 0\\
0 & -1
...
sta odbojnost in prepustnost neodvisni od polarizacije. Sledi 
\begin{equation}
...
eksponentno z oddaljenostjo od mejne plasti. Vdorna globina odvisna od valovne 
...
na žice, je  inducirani tok bistveno manjši, saj je smer toka omejena
...
\end{equation}
kjer je $\underline{\epsilon}$ tenzor drugega ranga in ima v splošnem devet komponent.
...
Za poljubno smer širjenja valovanja ter poljubno polarizacijo je račun razmeroma zapleten. 
...
Rešitev sistema najbolj nazorno predstavimo s ploskvijo valovnega vektorja, 
...
simetrično glede na os $z$. V optično enoosnem kristalu (desno) je le ena optična os, 
po dogovoru je to os $z$ (desno). Rdeče puščice označujejo polarizacijo.}
\label{kploskev}
\end{figure}
...
V tem primeru niti ni treba risati celotne ploskve, ampak zaradi rotacijske simetrije
...
Energijski tok tako potuje v smeri, ki ni enaka smeri valovnega vektorja. Smer
...
kot, pod katerim se lomi -- je v anizotropnih snoveh odvisna od polarizacije valovanja. V splošnem
...
pri prehodu, moramo popraviti \index{Lomni zakon}lomni zakon (enačba \ref{eq:lomni_zakon}).
...
(slika~\ref{foto:dvolom}) z medsebojno pravokotnima polarizacijama. 
...
valovanje loči na dva različno polarizirana žarka (a).
Optična os je usmerjena pod poljubnim kotom glede na normalo mejne ravnine. Valovna vektorja
sta v tem primeru kolinearna, Poyntingova vektorja pa imata različne smeri (b).}
...
Z linearnim polarizatorjem pokažemo, da imata razmaknjeni sliki različni polarizaciji.}
\label{foto:dvolom}
\end{figure}

...
(\ref{eq:valovna-skalarna}), dobimo \index{Helmholtzeva enačba}Helmholtzevi
...
ravni val in hkrati reši Helmholtzevo enačbo (\ref{eq:Helmholtz}), je 
\begin{align}
...
\ref{eq:Poyntingov-vektor-c}) ter Maxwellovih enačb (\ref{eq:Maxwell1} -- \ref{eq:Maxwell4}) 
sledi\index{Intenziteta!ravni val}
\beq
...
\beq
U_P = -\frac{ik U_0 e^{-i\omega t}}{4\pi}\int\int\frac{e^{ik(r+r')}}{rr'}\left[\cos(\mathbf{n},
...
(Huygensov princip). \index{Huygensov princip}

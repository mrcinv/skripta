...
Pri dimenzijah odprtin $a>\lambda$ lahko uporabimo skalarno teorijo
\beq
...
$P$ s poljem na poljubni ploskvi ki obkroža točko $P$. Zvezo opisuje Kirchoffov integral \index{Kirchoffov integral}
\beq
U_P = -\frac{1}{4\pi}\int\int\left(U \mathbf{n}\cdot \nabla \frac{e^{ikr}}{r}-\frac{e^{ikr}}{r}\mathbf{n}\cdot \nabla U \right) dS,
\label{eq:Kirchoffov-integral}
\eeq
kjer je $\mathbf{n}$ normala na ploskev po kateri teče integral, $r$ je oddaljenost od točke P
do ploskeve $dS$. Kirchoffov integral velja splošno za katerokoli harmonično 
...
problem sipanja močno poenostavi. Vzemimo opdrtino poljubne oblike v za svetlobo
...
U = U_0 \frac{e^{ikr'}}{r'},
\eeq
kjer je $r'$ razdalja od izvora do točke na zasečitveni ploskvi (glej sliko XX).
Ploskev integracije zajema odprtino in objema točko $P$ v celoti, pri tem
pa moramo upoštevati še dva dodatna približka:
\begin{enumerate}
\item Valovna funkcija $U$ in njen gradient vplivata zelo malo na integral (njena vrednost je 0), razen v odprtini.
\item Vrednosti $U$ in njen gradient sta na odprtini podana z poljem, ki ga izseva izvor v točki $S$ in sta torej takšna, kot bi bila, če zaslona ne bi bilo.
\end{enumerate}
...
\index{Fresnel-Kirchhoffov uklon}

\begin{definition}
\label{naloga-Fresnel-Kirchhoff-uklon}
Pokaži, da z uporabo Kirchoffovega integrala in predpostavki, da je polje na odprtini
...
\beq
U_P = -\frac{ik U_0 e^{-i\omega t}}{4\pi}\int\int\frac{e^{ik(r+r')}}{rr'}\left[\cos(\mathbf{n},\mathbf{r})-\cos(\mathbf{n},\mathbf{r'})\right] dS,
\label{eq:Fresnel-Kirchoffov-integral}
\eeq
\end{definition}


Ta zveza ni nič drugega kot matematična formulacija Huygensovega principa. To najlažje
...
U_P =  -\frac{ik}{4\pi} \int \int\frac{U_S e^{ik(r)}}{r}\left[ \cos(\mathbf{n},\mathbf{r})-1\right] dS
\eeq
kjer $U_S$ predstavlja kompleksno amplitudo polja v odprtini
\beq
U_S = U_0 \frac{e^{ikz}}{z}=E_0 e^{ikz},
\eeq 
...
kompleksno maplitudo
\beq
U = U_0 \frac{e^{ikr}}{r} dS,
\eeq 
ter da je polje v izbrani točki $P$ vsota prispevkov posameznih krogelnih valovanj
(Huygensov pricip). \index{Huygensov princip}
Za razliko od osnovnega Huygensov pricipa, v enačbi \ref{eq:Fresnel-Kirchoffov-integral} 
...
\beq
U_P = U_0\frac{J_1(\pi a \rho/ \lambda z)}{\pi a \rho/ \lambda z},
\eeq

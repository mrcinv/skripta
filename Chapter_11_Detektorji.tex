...
Največ pozornosti bomo posvetili danes najbolj razširjenim polprevodniškim 
detektorjem. Na koncu bomo spoznali še šum pri detekciji, ki omejuje najmanjši 
...
Po drugi strani pa je odziv termičnih detektorjev neodvisen
od valovne dolžine vpadne svetlobe, zaradi česar so termični detektorji uporabni na 
širokem območju od globokega ultravijoličnega do daljnega infrardečega dela svetlobe. 
...
signalom ($S$ --{\it signal}) in šumom ($N$ -- {\it noise}) enako $S/N = 1$. 
...
(\ref{piro}) in izračunajmo tok $I$ v odvisnosti od krožne frekvence modulacije.
\begin{equation}
...
kje $n_F$ označuje število vpadnih fotonov v času $t$. 
...
iz dinod izbijajo dodatne elektrone in izhodni signal se močno ojači.}
\label{fig:PMT}
\end{figure}
...
Z dopiranjem povečamo območje delovanja detektorja v infrardečem delu spektra. }
...
V vidnem in bližnjem infrardečem delu svetlobe se 
...
polprevodnika ($p$-$i$-$n$ fotodioda). Ko svetloba vpada na spoj različno dopiranih polprevodnikov,
...
Silicijeve \index{Silicij} fotodiode so tako uporabne za zaznavanje valovnih dolžin do največ okoli
...
Ob spoju tako nastane ozek pas, imenujemo ga izpraznjeni sloj, kjer ni več 
...
elektrone in vrzeli od spoja in v ravnovesju se Fermijeva energija izenači. Potencialni
skok je približno enak $\Delta E \approx E_d-E_a$, kar je le malo manj od 
širine reže $E_g$ (slika~\ref{11_PN1}\,c). Tipična jakost električnega polja na spoju
je $10^5$--$10^7~\si{V/m}$.
...
\caption{Shema energijskih nivojev v $p$- (a) in $n$-tipu (b) polprevodnika ter na spoju $p$-$n$ (c), 
...
Priključimo na tako diodo napetost $U$, tako da je pozitivna na $p$ strani diode. Takrat 
...
Celotni tok skozi spoj $p$-$n$ je sestavljen iz prispevkov elektronov in vrzeli. Opiše ga
...
razmerje \index{$SNR$!see {Razmerje signal proti šumu}}
...
Zaradi kvadratne zveze med električim tokom in električno močjo velja 
...
Načinov izpeljave termičnega šuma na uporniku je več. Mi ga razložimo na preprostem vezju, 
v katerem sta povezana upornik z upornostjo $R$ in kondenzator s kapaciteto $C$. Izvorov 
napetosti ni, pojavljajo pa se fluktuacije v napetosti $\Delta U$. Tako
vezje ima eno prostostno stopnjo in to je napetost, ki se pojavi zaradi šuma. Iz termodinamike
vemo, da je povprečna energija sistema, ki je v ravnovesju, $kT/2$ na vsako prostostno
stopnjo. Energija na kondenzatorju je pri napetosti $\Delta U$ tako enaka
\begin{equation}
\overline{W} = \frac{1}{2}C (\Delta U)^2 = \frac{1}{2}kT.
\end{equation}
...
v infrardeče valovanje\index{Infrardeče valovanje} ali celo v vidno svetlobo. 
...
znatno sevajo. Detektorjem za infrardeče valovanje pogosto zmanjšamo aperturo 
...
infrardečem delu svetlobe, kjer pomembno vpliva šum ozadja, in pri zelo nizkih intenzitetah 
...
le pri razliki krožnih frekvenc $\omega_s-\omega_r$. Sledi
\begin{equation}

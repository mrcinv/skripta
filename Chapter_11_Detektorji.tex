...
\caption{Shema energijskih pasov in zunanjega fotoefekta v kovini (a), 
ter polprevodniku (b) in notranjega fotoefekta v polprevodniku (c). 
...
Svetloba vpada na fotokatodo, zaprto v vakuumirani stekleni bučki, in tam povzroči
fotoefekt. Izbite elektrone z zunanjo napetostjo pospešimo do anode 
in merimo električni tok, ki je sorazmeren s številom vpadlih fotonov. 

Območje detekcije je določeno z izstopnim delom kovine (oziroma
polprevodnika), iz katerega fotoni izbijajo elektrone. 
...
katere je katoda. Največje vrednosti, ki jih dosega, so okoli $40~\%$, pogosto pa več velikostnih redov manj. 
...
Pri fotocelicah pride pri končnih temperaturah tudi do spontane oddaje elektrona, zato 
nekaj toka teče tudi v popolni temi. To je temni tok fotodiode in tipično dosega
vrednosti okoli $10^{-15}~\si{\ampere}$, lahko pa tudi do več nA. Za občutljive meritve je treba vakuumsko fotodiodo hladiti. 
...
\section{Polprevodniške $p$-$n$ in $p$-$i$-$n$ fotodiode}
...
polprevodnika ($p$-$n$ fotodiode) ali pa je med njima še plast nedopiranega (intrinzičnega) polprevodnika 
($p$-$i$-$n$ fotodioda). Ko svetloba vpade na $p$-$n$ (ali $p$-$i$-$n$) stik, nastane par elektron-vrzel. Nosilca naboja 
zaradi električnega polja na stiku potujeta v različnih smereh. Odvisno od načina delovanja izmerimo
tok, ki steče skozi stik, ali napetost, ki se pojavi na stiku. 
Spektralni odziv fotodiod je seveda odvisen od energijske reže polprevodnika, iz katerega je fotodioda narejena.
...
Ponovimo najprej, kaj se zgodi, ko združimo $p$- in $n$- tip polprevodnika. Pri tem tip $p$ označuje
...
$p$-$i$-$n$ stiki so podobni, le da je med $p$ in $n$ dopiranima polprevodnikoma še plast
intrinzičnega, to je nedopiranega, polprevodnika. 
\begin{figure}[h]
\centering
\def\svgwidth{140truemm} 
...
Ko staknemo polprevodnik tipa $p$ s polprevodnikom tipa $n$ (in vmesno plastjo $i$), elektroni 
...
in negativno nabiti akceptorski atomi na strani $p$. Ti naboji povzročijo 
...
v prevodni smeri (levo) in v zaporni smeri (desno).}
\label{11_PNU}
\end{figure}

Priključimo na diodo napetost, tako da je pozitivna na $p$ strani diode. Takrat 
...
(slika\ref{11_PNU}). 
Celotni tok skozi $p$-$n$ stik je sestavljen iz prispevkov elektronov in vrzeli, opiše pa
ga tako imenovana karakteristična enačba diode (slika~\ref{11_IU})
\boxeq{11:dioda}{
I = I_0 (e^{e_0 U/kT}-1).
}
Pri tem $I_0$ označuje tok manjšinskih nosilcev naboja (zaporni tok ali tok nasičenja) 
in je navadno zelo majhen, okoli $10^{-5}$-$10^{-15}~\si{\ampere}$, pri čemer je eksponentno 
odvisen od temperature. Najmanjše vrednosti tako dosegamo le ob močnem hlajenju. 

Ista enačba velja tudi, če priključimo pozitivni pol na $n$ strani, negativni pa na $p$, če je
torej priključena napetost negativna. Takrat pravimo, da smo napetost priključili v zaporni smeri. 
Potencialna razlika se poveča in tok večinskih elektonov se zmanjša za faktor $\exp(-e_0 |U|/kT)$.

\begin{figure}[h]
\centering
\def\svgwidth{140truemm} 
\input{slike/11_PN1.pdf_tex}
\caption{$I(U)$ karakteristika fotodiode: neosvetljena (črna črta), osvetljena (modra črta) in 
dvakrat močneje osvetljena (rdeča črta).}
\label{11_IU}
\end{figure}
 
\subsection*{Osvetlitev $p$-$n$ stika}
Ko na polprevodnik vpade foton, ki ima energijo večjo od širine reže, lahko vzbudi elektron iz 
valenčnega v prevodni pas in nastane par elektron-vrzel. Če se to zgodi v izpraznjenem (ali
intrinzičnem) sloju $p$-$n$ stika, steče elektron pod vplivom električnega polja na stran $n$,
vrzel pa na stran $p$, tako da pride so sunka toka $2e_0$ v zaporni smeri. 
Karakteristiko fotodiode zato zapišemo kot (slika~\ref{11_IU})
\boxeq{11:fotodioda}{
I = I_0 (e^{e_0 U/kT}-1) - I_f,
}
pri čemer $I_f$ opisuje tok zaradi svetlobe in je enak
\boxeq{11:if}{
I_f = 2 e_0 \eta N_F = 2 e_0 \frac{\eta P}{h \nu}.
}
Z $N_F$ smo onačili število vpadnih fotonov na časovno enoto, $\eta$ je kvatni izkoristek,
$P$ pa označuje moč vpadne svetlobe. Vidimo, da svetlobni tok zgolj premika karakteristiko 
diode v vertikalni smeri, naraščajoča intenziteta svetlobe premika krivuljo proti negativnim vrednostim tokov. 
...
Če želimo, da je ozdiv detektorja linearen, motamo delati ptri u=0 ali U<0.
...
\subsection*{osvetljeno s svetlobo, kaj se zgodi}
...
para elektron-vrzel tok ne more steči. Zato se na diodi pojavi napetost. 
...
Tok v zaporni smeri močno naraste. Nosilni naboja, ki prečkajo izpraznjeno plast dobijo dovolj eergije, da
ionizirajo mrežne ione s trki. Nosilci, ki pri tem nastanejo, naprej ioniirajo ione in pride do t. i. plazu. 

...
Pri detekciji svetlobe je vedno prisoten šum. Beseda šum označuje naključne 
fluktuacije na izhodu iz detektorja, ki jih ne moremo ločiti od signala. Do izraza 
pridejo predvsem pri zaznavanju šibkih signalov svetlobe. 
Z različnimi pristopi lahko šum zmanjšamo, povsem ga pa ne moremo nikoli odpraviti.
Tako pri vsaki meritvi obstaja neka najmanjša vpadna moč svetlobe, ki 
jo še lahko ločimo od šuma, pod to vrednostjo pa se signal v šumu izgubi
(slika~\ref{11_sum}).
...
emisijo elektronov,
\item šum zaradi sevanja ozadja (okolica detektorja pri končni temperaturi vedno seva) 
in 
\item šum $1/f$ oziroma roza šum *** (nepojasnjen izvor, kaotični pojavi).
\end{enumerate}

\subsection*{Šum štetja} 
Ko govorimo o gostoti svetlobnega toka oziroma vpadni moči svetlobe na danem detektorju, 
ne smemo pozabiti, da je svetloba sestavljena iz posameznih fotonov: enkrat jih vpade malo
več, drugič malo manj. Moč, o kateri govorimo, je dejansko povprečna moč $\overline{P}$ in število 
vpadlih fotonov na časovno enoto je povprečna vrednost števila vpadlih fotonov na časovno enoto
$\overline{n}_F = \overline{P}/h\nu$. 
Povprečno število fotoelektronov, ki se pojavijo v časovnem intervalu $\tau$, je tako
\beq
\overline{n} = \frac{\overline{N}}{\tau} = \frac{\overline{P}}{h \nu},
\eeq
pri čemer smo privzeli, da ima detektor popoln kvantni izkoristek, sicer bi morali izraz 
pomnožiti še z $\eta$. Ker gre pri vpadu fotonov 
za diskretne in neodvisne procese, verjetnost, da v intervalu dolžine (času merjenja) 
$\tau$ vpade
$N$ fotonov, opišemo s Poissonovo porazdelitvijo (slika~\ref{11_Poiss}).
Verjetnost, da bomo v času $\tau$ zaznali $N$ fotoelektronov, je tako
\beq
p(N) = \frac{\overline{N}^N e^{-\overline{N}}}{N!},
\label{Poisson}
\eeq
pri čemer je $\overline{N} = \overline{n}\tau$.
\begin{figure}[h]
\centering
\def\svgwidth{80truemm} 
%\input{slike/11_Poiss.pdf_tex}
\caption{Poissonova porazdelitev verjetnosti za tri različne vrednosti $\overline{N}$.}
\label{11_Poiss}
\end{figure}
\begin{definition}
Pokaži, da je vrh Poissonove porazdelitve (enačba~\ref{Poisson}) pri $N \approx \overline{N}$
in varianca $\overline{(N-\overline{N})^2} = \overline{N^2}-\overline{N}^2 = \overline{N}$.
\label{nal:Poiss}
\end{definition}
Število fotonov, ki vpadejo na detektor v danem časovnem intervalu, označimo z $\Delta N = N-\overline{N}$.
V povprečju je ta vrednost seveda enaka nič, zato sta bolj merodajni količini varianca, 
ki je enaka (glej nalogo~\ref{nal:Poiss})
\beq
...
\label{sigma}
\eeq
Standardni odklon, ki predstavlja šum, torej narašča z naraščajočim $\overline{N}$. 
Vendar nas absolutni šum večinoma ne zanima, saj je pri detekciji svetlobe ključno razmerje 
signal proti šumu, ki ga označimo s $SNR$ ({\it Signal to Noise Ratio}-- razmerje med signalom in šumom)
\footnote{Pogosto se uporablja tudi oznako $S/N$. Tukaj smo jo 
zaradi jasnosti zamenjali, saj $N$ označuje število fotonov oziroma elektronov.}.
\beq
SNR = \frac{\overline{N}}{\sigma} = \sqrt{\overline{N}}.
\eeq
Vidimo, da razmerje signal proti šumu narašča z naraščajočim svetlobnim tokom, torej relativni šum
pojema. Za primer poglejmo dva primera detekcije. 
V prvem je povprečno število vpadlih fotonov v danem časovnem intervalu $10^6$, v drugem pa le $100$. 
Pri vpadu močnejšega signala na detektorju zaznavamo $10^6 \pm 1000$ fotonov, pri vpadu šibkejšega
pa $100 \pm 10$. Čeprav je absolutni šum v prvem primeru znatno večji, je relativni šum stokrat manjši.
Za zmanjšanje šuma štetja mora biti torej signal kar se da velik. 

\begin{remark}
Razmerje signal proti šumu $SNR$ lahko vpeljemo na dva načina. Prvi je ta, ki smo ga uporabili mi, pri 
katerem velja $SNR = \overline{N}/\sigma = \sqrt{\overline{N}}$. V tem primeru gre za $SNR$ 
optične moči oziroma števila fotonov ali elektronov in s tem povezanega električnega toka. 
Lahko pa vpeljemo tudi $SNR_e$ električne moči, ki je, zaradi kvadratne zveze med električno močjo in 
električnim tokom, enak $SNR_e=SNR^2=\overline{N}$.
\end{remark}

Zapišimo zdaj fluktuacije izhodnega električnega toka iz detektorja, pri čemer upoštevajmo 
enačbo~(\ref{varianca})
\beq
\overline{\Delta I^2}=\overline{(I-\overline{I})^2} = \overline{(N-\overline{N})^2}\,\frac{e_0^2}{\tau^2} = 
\overline{N}\,\frac{e_0^2}{\tau^2}= \overline{I}\,\frac{e_0}{\tau}.
\eeq
Vpeljemo še pasovno širino detekcije $\Delta\nu_B = 1/(2\tau)$ in dobimo
\boxeq{11:sum}{
\overline{\Delta I^2} = 2 \overline{I}\,e_0\, \Delta\nu_B.
}
Razmerje signala proti šumu potem zapišemo kot
\beq
SNR = \frac{\overline{I}}{\sqrt{\overline{\Delta I^2}}}= \frac{\overline{I}}{\sqrt{2 \overline{I}\,e_0\, \Delta\nu_B}} 
\eeq
Zanima nas vrednost vpadne moči svetlobe, pri kateri je šum enako velik signalu
To se zgodi pri vpadni moči $P_{\mathrm NEP}$, pri čemer NEP pomeni {\it noise-equivalent-power} 
-- moč, ki ustreza šumu.
Zapišemo jo kot
\beq
P_{\mathrm NEP} = 
